...
namespace Aplicacion.Validaciones
{
    public static class CustomHelpers
    {
        ...
        /// <summary>
        /// Obtiene que la transaccion sea consistente con el saldo de la cuenta y devuelve el tipo de transaccion
        /// </summary>
        ...
        /// <summary>
        /// Valida que la transaccion no exceda el cupo diario
        /// </summary>
        /// <param name="Valor">Valor de la transaccion</param>
        /// <param name="cuentaId">Numero de cuenta</param>
        /// <param name="_context">Contexto de la base de datos</param>
        /// <exception cref="ManejadorExcepcion"></exception>
        public static void ValidarSaldo(decimal Valor, Guid cuentaId, BcoDbContext _context)
        {
            if (Math.Abs(Valor) == 0)
            {
                throw new ManejadorExcepcion(HttpStatusCode.Unauthorized, new { mensaje = MSG_EX.TRANSACCION_CERO });
            }
            else
            {
                if (Valor < 0)
                {
                    var debitosDelDia = _context.Movimiento
                        .Where(x => x.CuentaId == cuentaId
                            && x.Fecha.Date == DateTime.UtcNow.AddHours(-5).Date
                            && x.TipoMovimiento == TipoMovimiento.Debito.ToString())
                            .Sum(x => x.Valor);

                    if ((debitosDelDia + Math.Abs(Valor)) > Validaciones.CustomHelpers.limiteDiario)
                    {
                        throw new ManejadorExcepcion(HttpStatusCode.Unauthorized, new { mensaje = string.Format(MSG_EX.CUPO_DIARIO_EXEDIDO, Math.Abs(Valor).ToString(), debitosDelDia.ToString()) });
                    }
                }
            }
        }
